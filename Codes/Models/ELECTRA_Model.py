...
# 4. ELECTRA Tokenizer ve Modeli yükle
...
inputs = tokenizer(
    ...
    padding=True,
    truncation=True,
    max_length=512,  # ELECTRA için max_length 512 önerilir
    return_tensors="pt"
)
...
for epoch in range(epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        counter = 0
        torch.save(model.state_dict(), 'best_model_electra.pth')
    else:
        counter += 1
        if counter >= patience:
...
end_time = time.time()
print(f"\nToplam Eğitim Süre: {end_time - start_time:.2f} saniye")
